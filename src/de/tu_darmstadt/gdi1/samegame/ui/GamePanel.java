...
/**
 * Class representing a basic game field display that you can extend for your
 * own solution by deriving from it.
 * 
...
public abstract class GamePanel extends JPanel implements MouseListener {
	...
	public Vector<JButton> getEntities() {
		return entities;
	}
	 ...
	 * 
	 * @param Autosize
	 *            True if automatic sizing shall be enabled, otherwise false
	 */
	...
	protected void registerImage(String identifier, String fileName)
		...
		/*	ImageIcon icon;
			...
			*/
			...
			if (System.getProperty("os.name").toLowerCase().matches(
					".*windows.*")) {
				builder.append("file:///");
			} else {
				builder.append("file://");
	...
	protected JButton placeEntity(Icon icon) {
		...
		//if (icon.equals(images.get("empty")))
		//	btn.setVisible(false);
